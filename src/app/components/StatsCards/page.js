  ...
  if (!data) {
    return (
      <div className="text-center py-6">
        <p className="text-gray-500">Loading channel stats...</p>
      </div>
    )
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border-l-4 border-red-500">
        <div className="text-xl sm:text-2xl font-bold text-gray-900">
          {data?.subscribers?.toLocaleString() || 0}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between py-2 border-b border-gray-100">
              ...
              <span className="text-gray-900 font-mono text-xs sm:text-sm truncate ml-2">
                {data?.channelId || "N/A"}
              </span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100">
              ...
              <span className="text-gray-900 text-sm sm:text-base">
                {data?.creationDate ? new Date(data.creationDate).toLocaleDateString() : "N/A"}
              </span>
            </div>
          </div>
          <div className="space-y-2 sm:space-y-3">
